...
class SimController {
 public:
  SimOutput update(const SimSample &s, float hour, bool valid = true) {
    SimOutput o{};
    if (!valid || !s.valid) {
      aerator_on_ = true;
    ...
    if (s.battery_v < 6.8f) {
      o.reason = "BATTERY_LOW";
    }

    if (s.do_mg_l < 4.0f) {
      aerator_on_ = true;
      o.reason = "DO_LOW";
    } else if (s.nh3 >= 0.5f && o.score >= 70.0f) {
      aerator_on_ = true;
      o.reason = "HIGH_COMBINED_RISK";
    } else if (o.score >= 70.0f) {
      aerator_on_ = true;
      o.reason = "CRASH_SCORE_HIGH";
    } else if (s.do_mg_l > 5.5f && o.score < 45.0f) {
      aerator_on_ = false;
      if (s.battery_v >= 6.8f) {
        o.reason = "NORMAL";
      }
